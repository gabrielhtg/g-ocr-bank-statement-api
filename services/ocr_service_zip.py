...
from services.utils import calculate_distance_between_2_points, order_points, get_value_percentage, contains_number
...
def proceed_ocr_zip(file_name_array, app):
    list_baris = []
    list_sub_data = []
    data_baris = []
    data_baris_temp = []
    ...
    for i in file_name_array:
        ...
        for t in text_:
            ...
            if save_data:
                ...
                if halaman == len(file_name_array) and 'saldo' in str(ocr_text).lower():
                    data_baris_temp = data_baris.copy()
                    wait_for_awal = True

                if halaman == len(file_name_array) and 'awal' in str(ocr_text).lower() and wait_for_awal:
