...
def bniAnalysisData (transactionData) :
    freqDebit = 0
    arrDebit = []
    ...
    for e in transactionData :
        if e['debit_credit'] == 'D' :
            freqDebit += 1
            convertedDebet = float(e['amount'].replace(',', '').replace('.', '').replace('Rp ', '')) / 100
            arrDebit.append(convertedDebet)
            
        
        if e['debit_credit'] == 'K' :
            freqKredit += 1
            convertedKredit = float(e['amount'].replace(',', '').replace('.', '').replace('Rp ', '')) / 100
